...
function mergeObjects(objects) {
  const mergedObj = objects
    .reduce((acc, obj) => acc.concat(Object.entries(obj)), [])
...
function makeWord(lettersObject) {
  const word = Object.entries(lettersObject).reduce(
    ...
    queue.reduce(
      (acc, bill) => {
        ...
        if (bill === 25) {
          ...
          if (acc.fifty > 0 && acc.twentyFive > 0) {
            acc.fifty -= 1;
            acc.twentyFive -= 1;
          } else if (acc.twentyFive >= 3) {
            acc.twentyFive -= 3;
          } else {
            return false;
          }
...
class CssSelector {
  ...
  pseudoElement(value) {
    if (this.used.pseudoElement) {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
    this.used.pseudoElement = true;
...
const cssSelectorBuilder = {
  ...
  combine(selector1, combinator, selector2) {
    ...
    combinedSelector.selector = combined;
